...
"From TID, From JID,   To TID,   To JID,    Sus. min, Sus. max,  Signal at \n"
"      65,       68,       65,       72,           0,        0, completion \n"
"      65,       68,       65,       64,           0,        0,      start \n"
;
...
"From TID, From JID,   To TID,   To JID,    Sus. min, Sus. max,  Signal at \n"
"      65,       68,       65,       72,           0,        0, completion \n"
"      65,       72,       65,       69,           0,        0, completion \n"
"      65,       69,       65,       44,           0,        0,      start \n"
"      65,       68,       65,       64,           0,       61,      start \n"
;
...
TEST_CASE("[global-prec] taskset-17 check transitivity pessimism (1)") {
	...
	CHECK(space->is_schedulable());
	delete space;
	prob.prec[3] = NP::Precedence_constraint<dtime_t>(jobs[0].get_id(), jobs[3].get_id(), Interval<dtime_t>(0, 62), false);
	validate_prec_cstrnts(prob.prec, prob.jobs);
	space = NP::Global::State_space<dtime_t>::explore(prob, {});
	CHECK(!space->is_schedulable());
	delete space;
...
TEST_CASE("[global-prec] taskset-18 check transitivity pessimism (2)") {
	...
	CHECK(space->is_schedulable());
	delete space;
	prob.prec.push_back(NP::Precedence_constraint<dtime_t>(jobs[2].get_id(), jobs[3].get_id(), Interval<dtime_t>(0, 0), false));
	validate_prec_cstrnts(prob.prec, prob.jobs);
	space = NP::Global::State_space<dtime_t>::explore(prob, {});
	CHECK(space->is_schedulable());
	delete space;
	prob.prec[4] = NP::Precedence_constraint<dtime_t>(jobs[2].get_id(), jobs[3].get_id(), Interval<dtime_t>(0, 1), false);
	validate_prec_cstrnts(prob.prec, prob.jobs);
	space = NP::Global::State_space<dtime_t>::explore(prob, {});
	CHECK(!space->is_schedulable());
	delete space;
...
"From TID, From JID,   To TID,   To JID,    Sus. min, Sus. max,  Signal at \n"
"       0,        0,        1,        1,           0,        0, completion \n"
"       0,        0,        2,        2,           0,        0, completion \n"
"       1,        1,        3,        3,           0,        0,      start \n"
;
