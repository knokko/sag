...
namespace NP::Reconfiguration {
    template<class Time> static void print_schedulability_info(const Global::State_space<Time> *space) {
        long megabytes = get_peak_memory_usage() / 1024;
        std::cout << "    intermediate exploration: schedulable? " << space->is_schedulable() << " time: " << space->get_cpu_time() << " peak memory up until now: " << megabytes;
    ...
    template<class Time> static bool is_schedulable(const Scheduling_problem<Time> &problem, bool print_info) {
        ...
        const bool result = space->is_schedulable();
