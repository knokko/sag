...
namespace NP::Reconfiguration {
	...
	template<class Time> static std::vector<Job_index> find_safe_job_ordering(
		...
		if (safe_path.empty()) {
			if (!options.save_job_ordering.empty()) {
				std::cout << " I will give up" << std::endl;
				exit(2);
			}
			made_safe_path_from_scratch = true;
		...
		if (!options.save_job_ordering.empty()) {
			FILE* file = fopen(options.save_job_ordering.c_str(), "wb");
			if (file == NULL) {
			...
			fflush(file);
			fclose(file);
			std::cout << "Safe job ordering was saved to " << options.save_job_ordering << std::endl;
			exit(1);
		}
